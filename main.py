...
from subscription import send_message_to_sqs
from subscription import subscribe_email
from subscription import unsubscribe_email
...
async def delete_file(name: str, session: Session = Depends(get_session)):
    ...
    if image:
        ...
        raise HTTPException(
            status_code=400, detail=f"Image item with name '{name}' not found",
        )
