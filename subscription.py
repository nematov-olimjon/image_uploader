...
def unsubscribe_email(email: str) -> None:
    try:
        ...
        for subscription in subscriptions["Subscriptions"]:
            if email == subscription["Endpoint"]:
                if subscription["SubscriptionArn"] ==  "Deleted":
                    raise HTTPException(status_code=400, detail=f"This email address has been deleted!")
                elif subscription["SubscriptionArn"] ==  "PendingConfirmation":
                    raise HTTPException(status_code=400, detail=f"Please confirm subscription first!")
        ...
        raise HTTPException(status_code=400,
                                detail=f"Error occurred: {e.detail}")


def send_message_to_sqs(message):
    sqs.send_message(QueueUrl=os.environ.get("SQS_QUEUE_NAME"),
                                  MessageBody=message)
